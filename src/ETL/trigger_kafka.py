# IMPROVED PRODUCER (trigger_kafka.py)
...
def main():
    ...
    with MySQLConnect(config["mysql"].host, config["mysql"].port, config["mysql"].user,
        ...
        producer = KafkaProducer(
            ...
            batch_size=16384,  # Optimize batch size
        ...
        while True:
            try:
                ...
                if data:
                    print(f"\n📦 Processing batch {batch_id} with {len(data)} records...")
                ...
                for record in data:
                    ...
                    enriched_record = {
                        ...
                        "log_timestamp": record["formatted_timestamp"],  # Use formatted timestamp
                    ...
                    producer.send("vnontop", enriched_record)
